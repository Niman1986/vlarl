    ...
    *,
    ...
    if use_cot:
        if os.path.isfile(reasoning_dataset_path):
            ...
            raise FileNotFoundError(f"Dataset file `{reasoning_dataset_path}` not found.")

        with open(reasoning_dataset_path, "r") as f:
            reasoning_dataset = json.load(f)

    def make_tf_dict(raw_dict):
        cprint("Building the reasoning dict...", "yellow")
        keys = []
        values = []
        ...
        def reasoning_dict_to_str(d):
            ...
            reasoning_parts = []
            for tag in tags:
                if database_keys[tag] in d:
                    reasoning_parts.append((tag, d[database_keys[tag]]))
        ...
        for file_name in raw_dict.keys():
            if "reasoning" not in raw_dict[file_name].keys():
                has_reasoning[0] += 1
                continue
            else:
                has_reasoning[1] += 1
            len_traj = len(raw_dict[file_name]["reasoning"])
            for i in range(len_traj):
                keys.append(file_name + "_" + str(i))
                reasoning_dict = raw_dict[file_name]["reasoning"][str(i)]
    ...
    def restructure(traj):
        ...
        # traj_id = traj["_traj_index"][0]
        # traj_id = tf.as_string(traj_id)        
        # reasonings = reasoning_dataset.lookup(file_name + "_" + traj_id)

        file_name = traj["traj_metadata"]["episode_metadata"]["file_path"][0]
        traj_id = traj["_traj_index"][0]

        file_names = tf.repeat(file_name, traj_len)
        episode_ids = tf.as_string(tf.repeat(traj_id, traj_len))
        
        if use_cot:
            reasonings = reasoning_dataset.lookup(file_names + "_" + episode_ids)
            traj = {
                "observation": new_obs,
                "task": task,
...
def make_interleaved_dataset(
    ...
    *,
    ...
    for dataset_kwargs, threads, reads in zip(
        ...
        dataset_kwargs["reasoning_dataset_path"] = reasoning_dataset_path
...
if __name__ == "__main__":
    _ = make_dataset_from_rlds(
        name="libero_10_no_noops",
        data_dir="./data/modified_libero_rlds/",
        train=True,
        reasoning_dataset_path="./reasonings_formatted.json",
    )

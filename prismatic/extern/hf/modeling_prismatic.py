...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def predict_action_cot(
        ...
        # If the special empty token ('') does not already appear after the colon (':') token in the prompt
        # (after "OUT:" or "ASSISTANT:"), insert it to match the inputs seen at training time
        # if not torch.all(input_ids[:, -1] == 29871):    # NOTE: chain-of-thought needs this, cause ECOT doesn't have this. Turn on it to output action only (noqa).
        #     input_ids = torch.cat(
        #         (input_ids, torch.unsqueeze(torch.Tensor([29871]).long(), dim=0).to(input_ids.device)), dim=1
        #     )
        ...
        # Run VLA inference
        # generated_ids = self.generate(input_ids, max_new_tokens=self.get_action_dim(unnorm_key), **kwargs)
        generated_ids = self.generate(input_ids, max_new_tokens=1024, **kwargs)  #

        # Extract predicted action tokens and translate into (normalized) continuous actions
        predicted_action_token_ids = generated_ids[0, -(self.get_action_dim(unnorm_key) + 1) : -1].cpu().numpy()    # ref: https://github.com/MichalZawalski/embodied-CoT/issues/2
        discretized_actions = self.vocab_size - predicted_action_token_ids
        discretized_actions = np.clip(discretized_actions - 1, a_min=0, a_max=self.bin_centers.shape[0] - 1)
        normalized_actions = self.bin_centers[discretized_actions]

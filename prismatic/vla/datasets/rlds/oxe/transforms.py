...
def place_the_red_cube_into_blue_bowl5_hz_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    # Process action
    action = trajectory["action"]
    # Assume the last dimension of action is gripper action, range from -1 (open) to 1 (close)
    gripper_action = action[..., -1:]
    # Convert gripper action from -1/1 to 0/1 and invert, so 1 means open, 0 means close
    gripper_action = 1 - ((gripper_action + 1) / 2)
    # Update action
    trajectory["action"] = tf.concat([action[..., :-1], gripper_action], axis=-1)

    # Process observation
    observation = trajectory.get("observation", {})

    # Extract end effector state (position and orientation) and gripper state
    # Using actual keys from dataset: 'joint_states', 'ee_pos', and 'ee_ori'
    observation["eef_position"] = observation.get("ee_pos", tf.zeros((3,), dtype=tf.float32))
    observation["eef_orientation"] = observation.get("ee_ori", tf.zeros((3,), dtype=tf.float32))
    observation["gripper_state"] = observation.get("joint_states", tf.zeros((2,), dtype=tf.float32))[:2]

    # Updated observation is included in the trajectory
...
OXE_STANDARDIZATION_TRANSFORMS = {
    ...
    ### Real-world NTU datasets
    "place_the_red_cube_into_the_blue_bowl200": place_the_red_cube_into_blue_bowl5_hz_dataset_transform,
}

...
def convert_openvla_weights_to_hf(cfg: HFConvertConfig) -> None:
    ...
    # hf_config.text_config.vocab_size = len(tokenizer)
    assert tokenizer.pad_token_id == hf_config.pad_token_id, f"Incorrect Pad Token ID! tokenizer: {tokenizer.pad_token_id}, config: {hf_config.pad_token_id}"
    # assert len(tokenizer) > hf_config.text_config.vocab_size, "Tokenizer vocabulary must be larger than LLM vocabulary!" # FIXME: 151921 vs. 151936

    # action_tokenizer = ACTION_TOKENIZERS["extra_action_tokenizer"](tokenizer)

    # Patch LLM Config in `hf_config` with vocab_size (+ `hf_config.pad_to_multiple_of`), pad_token_id + validate
    # hf_config.text_config.vocab_size += hf_config.pad_to_multiple_of  # FIXME: mismatch
    ...
    print(f"[*] Loading Vision Backbone [bold]{prismatic_config['vision_backbone_id']}[/]")
    vision_backbone, image_transform = get_vision_backbone_and_transform(
        prismatic_config["vision_backbone_id"],
        prismatic_config["image_resize_strategy"],
    )
    ...
    # Add vision backbone state dict to model state dict
    model_state_dict["vision_backbone"] = vision_backbone.state_dict()

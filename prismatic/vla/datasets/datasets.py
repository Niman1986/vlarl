...
IGNORE_INDEX = -100     # -100 is the default ignore_index in PyTorch’s CrossEntropyLoss
...
class VanillaTransform:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        """[Used by RL] Leave the tokenization part for agent."""
        dataset_name, action = rlds_batch["dataset_name"], rlds_batch["action"][0]
        img = Image.fromarray(rlds_batch["observation"]["image_primary"][0])
        lang = rlds_batch["task"]["language_instruction"].decode().lower()

        # answer = self.action_tokenizer(action)

        return dict(img=img, lang=lang, action=action, dataset_name=dataset_name, rlds_batch=rlds_batch)
...
class RLDSBatchTransform:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        dataset_name, action = rlds_batch["dataset_name"], rlds_batch["action"][0]
        img = Image.fromarray(rlds_batch["observation"]["image_primary"][0])    # [224, 224, 3]
        lang = rlds_batch["task"]["language_instruction"].decode().lower()
        if self.use_cot:
        ...
        # num_answer_tokens = len(raw_action_tokens)    # NOTE: llama2 tokenizer will add the start token '1' and special token '29871'
        num_answer_tokens = 7
        ...
        if self.print_prompt_limit > 0:
            ...
            # print("Prompt:", p)
            cprint("Conversation:", "cyan")
            print(conversation)
            p = prompt_builder.get_prompt()
            cprint("Prompt:", "yellow") # include In:, OUT:, and ACTION:
            print(p)

            self.print_prompt_limit -= 1

        # DEBUG: save lang and pixel_values
        # save_path = "debug"
        # os.makedirs(save_path, exist_ok=True)
        # save_lang = prompt_builder.get_prompt()
        # save_img = img
        # save_img.save(f"{save_path}/image.png")
        # with open(f"{save_path}/lang.txt", "w") as f:
        #     f.write(save_lang)
        ...
        input_ids, labels = torch.tensor(input_ids), torch.tensor(labels)   # include '2' as eos token
        ...
        if not self.predict_stop_token: # default: True
...
class RLDSDataset(IterableDataset):
    def __init__(
        ...
        use_cot: bool = False,
        reasoning_dataset_path: str = None,
    ) -> None:
